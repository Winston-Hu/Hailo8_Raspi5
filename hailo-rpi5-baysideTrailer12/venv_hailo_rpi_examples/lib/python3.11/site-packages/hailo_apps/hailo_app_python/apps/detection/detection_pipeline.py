...
class GStreamerDetectionApp(GStreamerApp):
    def __init__(self, app_callback, user_data, parser=None):
        ...
        # use --score-threshold to change confidence in terminal
        parser.add_argument(
            "--score-threshold",
            type=float,
            default=0.3,
            help="Score threshold for hailodetection NMS (maps to nms-score-threshold)"
        )
        ...
        self.batch_size = 2
        src_str = str(self.video_source)

        if isinstance(self.video_source, str) and src_str.lower().endswith((".jpg", ".jpeg", ".png", ".bmp")):
            self.batch_size = 1
            print(f"[INFO] Detected image input: {src_str} -> using hailonet batch_size={self.batch_size}")
        else:
            print(f"[INFO] Using hailonet batch_size={self.batch_size} for source: {src_str}")

        # nms_score_threshold = 0.3
        nms_score_threshold = self.options_menu.score_threshold
